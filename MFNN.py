...
Needs to do: add ploting at the end
"""
import torch
...
class Model(nn.Module):
    ...
    def forward(self, x_l, x_h):
        ...
        y_h = self.net_h_l(torch.cat((y_l_h, x_h), dim=1))
        return y_l, y_h
...
def data_loader(device):
    ...
    data = scipy.io.loadmat('./mfdata2.mat')
    ...
    id_h = np.random.choice(xh.shape[0], Nh, replace=False)
